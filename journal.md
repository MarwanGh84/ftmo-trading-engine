...
## 2026-07-15 21:50 Dubai — CRITICAL: engine currently FROZEN (sticky)
Post-EOD audit reconcile found `state.json` shows `frozen=True, frozen_reason="cTrader unreachable 36 cycles", frozen_sticky=True` — DESPITE the audit itself completing cleanly with live authenticated cTrader data (bal $9,879.84 matches). Watchdog log confirms intermittent bridge connection-refused errors on port 9876 through this evening (streak climbed to 38), but per design a connectivity freeze should be non-sticky and auto-clear on reconnect — it isn't clearing here. Net effect: the `not_frozen` rail is blocking ALL new order placement right now, and will keep blocking tomorrow's sessions until manually cleared. Did not unfreeze (out of scope for this read-only review — engine writes are not this run's job). **Recommend the user run `~/trading/bin/ftmo unfreeze --force` once cTrader connectivity is confirmed stable**, ideally before the 09:32 Thu morning brief.

## 2026-07-16 11:00 Dubai — London Open Session
Engine still frozen (sticky) from last night's 21:50 flag — not_frozen rail blocks all new placement, so no execute attempted regardless of setup quality (avoids burning request budget on guaranteed refusals). Flat, 0 pending/open (both stale GBPJPY/CADJPY orders from 07-13/07-14 are gone — cleared since last check). Scanner candidates were mostly range-regime bull-near-resistance (weak continuation pattern, skip default); checked the two trend-regime ones (GBPAUD uptrend, USDCAD downtrend) — both sitting right at a fresh breakout extreme with zero retest, same chasing pattern skipped all week. 2 shadow skips logged. **Recommend user run `~/trading/bin/ftmo unfreeze --force` once cTrader connectivity is confirmed stable — now 2 consecutive sessions blocked.**

## 2026-07-16 13:32 Dubai — London Midday Session
Engine still frozen (sticky, "cTrader unreachable 36 cycles") — 3rd consecutive session blocked (since 07-15 21:50), despite audit itself pulling live authenticated data cleanly each time (bal $9,879.84 unchanged, 0/5 fills, flat). No execute attempted (guaranteed refusal). Reviewed the two trend-regime scanner candidates: USDCAD (bear, 20D low 1.40247) is grinding sideways 1.4038-1.4054 for 24h+ with no rejection/retest structure — still just sitting at the extreme, not a clean entry. GBPAUD (bull, 20D high) is mid-pullback from yesterday's breakout with no defined support level yet, and AUD is COT crowded_short — avoid adding to that crowded direction. Both shadow-logged as skips. Remaining 7 candidates are the same range-regime bull-near-resistance JPY/GBPUSD/NZDUSD chase cluster flagged repeatedly this week — skipped without individual logging (trivial, no new info). GBP GDP beat (0.1% vs 0.0%) released 06:00Z already priced in; next event risk is USD medium-impact cluster (Retail Sales/Philly Fed/Claims) at 12:30Z/16:30 Dubai, relevant to the 16:33 run. No new trade. **Unfreeze still needed — now 3 sessions lost to the sticky freeze bug.**

## 2026-07-16 16:33 Dubai — NY Overlap Session (final window)
Engine still frozen (sticky, "cTrader unreachable 36 cycles") — 4th consecutive session blocked since 07-15 21:50, despite audit pulling live authenticated data cleanly (bal $9,879.84 unchanged, 0/5 fills, flat, no discrepancies). No execute attempted. USDCAD (only trend-regime candidate) now grinding to a fresh 20D low (1.4025) with still no rejection/retest wick — same chase pattern, shadow-logged skip. Remaining 6 candidates are the unchanged range-regime JPY/NZDUSD bull-near-resistance cluster flagged all week — skipped without individual logging. Last trading window of the day closes flat, 0/5 fills, 0/2 poor. **Unfreeze still needed — now 4 sessions lost to the sticky freeze bug; recommend user run `ftmo unfreeze --force` after confirming cTrader connectivity before tomorrow's 09:32 morning brief.**
...
## 2026-07-23 11:00 Dubai — London Open Run
Still FROZEN ("cTrader unreachable 379 cycles") despite live/authenticated audit (bal $9,879.84 unchanged, flat, 0 discrepancies, 0/5 fills, 0/2 poor) — not_frozen rail guarantees refusal, so no execute attempted, no full 17-pair scan performed. Candidates queue had 7 flagged, all "bull/bear near 20D high/low" breakout-continuation chases in range or trend regimes (USDJPY/USDCHF/EURJPY/AUDJPY/CADJPY/GBPAUD/EURCHF) — none warranted a shadow log given continuation setups are the underperforming category (14-22% win) and frozen blocks placement regardless. COT: AUD/CAD/EUR crowded_short (all candidate directions were long-AUD/long-CAD/long-EUR, i.e. contrarian to crowd, not adding to it). Shadow edge holding +9pts (n=105 graded, take 31%/skip 22%). Unfreeze still not actioned by this session (write action outside this run's scope) — recommend `ftmo unfreeze --force` after a human confirms it's safe.

## 2026-07-23 13:34 Dubai — London Midday Session
Still FROZEN ("cTrader unreachable 379 cycles") despite audit completing clean/live/authenticated (bal $9,879.84 unchanged, flat, 0 discrepancies, 0/5 fills, 0/2 poor) — day 9, not_frozen rail guarantees refusal so no execute attempted, no full 17-pair scan run (would burn request budget for no actionable outcome). All 7 scanner candidates (USDJPY/USDCHF/EURJPY/AUDJPY/CADJPY/GBPAUD/EURCHF) are bull/bear-near-20D-extreme breakout chases with no pullback/rejection — the underperforming continuation category (14-22% win, shadow data) — none warranted a shadow log, matches week-long pattern. COT unchanged: AUD/CAD/EUR crowded_short, CHF/JPY neutral. Shadow edge holding +9pts (n=105 graded). No trade. **Unfreeze still needed — recommend `ftmo unfreeze --force` after connectivity re-confirmation; 9+ calendar days / 13+ sessions lost to this bug.**

## 2026-07-24 20:00 Dubai — EOD Review
**Bridge fully DOWN** — `ftmo eod --report` and `ftmo audit --report` both failed outright (connection refused, 127.0.0.1:9876, 6/6 retries) — same hard failure as 07-21, worse than the week-long sticky-FROZEN bug. No live reconciliation possible; cannot verify positions, weekend/CB exposure. Last known state (09:42 today): flat, bal $9,879.84 unchanged, 0/5 fills. One pending GBPJPY buy-limit (id 165481344, entry 218.30) unverified — flagged to Telegram since it's Friday (no weekend holds). Shadow edge unchanged +9pts (n=105 graded). Telegram-alerted manually. **Action needed: restart cTrader bridge; manually confirm GBPJPY pending order and flat status before weekend.**
